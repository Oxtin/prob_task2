...
#        self.prob_arr_gt_final,self.rule_whole_data = self.__find_rule(self.raw_df, self.__prob_arr_gt)
    ...
    def sample_evaluation(self, index_list):
    ...
    def raw_evaluation(self, gt_rule):
        rank_gt = self.prob_arr_gt_final[:,1,:].flatten()
        rank_gt[rank_gt==-1] = 0
        return ndcg_score(np.asarray([rank_gt]), np.asarray([rank_sample]), k=61) 

    def evaluation(self, sample_rank, gt_rank):
        return ndcg_score(np.asarray(sample_rank), np.asarray(gt_rank), k=61)   
    ...
    def get_gt_rule_final(self):
        return self.prob_arr_gt_final
